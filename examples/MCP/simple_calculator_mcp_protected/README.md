...
```bash
# Terminal 1
nat mcp serve --config examples/MCP/simple_calculator_mcp_protected/configs/config-server.yml
```
...
nat run --config examples/MCP/simple_calculator_mcp_protected/configs/config-client.yml \
...
```bash
# Terminal 2: Start the MCP calculator client as a server
nat serve --config examples/MCP/simple_calculator_mcp_protected/configs/config-client.yml
```
