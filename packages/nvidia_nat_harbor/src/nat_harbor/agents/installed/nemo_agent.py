...
class NemoAgent(HarborNemoAgent):
    ...
    async def _sync_inline_outputs_to_environment(self, environment: BaseEnvironment, output_path: Path,
                                                  trajectory_path: Path, stderr_path: Path) -> None:
    ...
    async def _run_library_mode(self, instruction: str, environment: BaseEnvironment) -> None:
        ...
        if env_type != "local":
            raise RuntimeError(
                "library_mode is currently supported only with local environment execution."
            )
